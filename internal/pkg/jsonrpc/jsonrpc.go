...
//
// from https://github.com/nubuki-all/rpc2 modified to avoid disconnected client on any error
package jsonrpc
...
func (c *jsonCodec) ReadHeader(req *rpc2.Request, resp *rpc2.Response) error {
	...
	if c.msg.Method != "" {
		...
		if c.clientResponse.Error != nil || c.clientResponse.Result == nil {
			...
			if err != nil {
				var data interface{}
				_ = json.Unmarshal(*c.clientResponse.Error, &data)
				return fmt.Errorf("invalid error %v", data)
			}
			resp.Err = &x
		}
	}
